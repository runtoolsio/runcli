...
def _init_job_parser(parent, subparser):
    ...
    dup_group = id_group.add_mutually_exclusive_group()
    dup_group.add_argument('-A', '--allow-duplicate', action='store_true',
                           help='Allow duplicate run ID. If the run already exists, creates a new execution '
                                'with incremented ordinal.')
    dup_group.add_argument('-S', '--suppress-duplicate', action='store_true',
                           help='Suppress duplicate run ID. Records the submission but immediately stops it. '
                                'Suppressed runs are hidden from default history views.')
    ...
    status_group.add_argument('--no-parse', action='store_true', default=False,
                              help='Disable output parsing (JSON, log patterns, key-value).')
    status_group.add_argument('--no-kv', action='store_true', default=False,
                              dest='no_parse', help=argparse.SUPPRESS)
    ...
    coord_group.add_argument('-c', '--checkpoint', metavar='PHASE_ID',
                             help='Adds checkpoint barrier with specified ID. Job waits until resumed via API or CLI. '
                                  'Timeout or stop terminates the job.')
